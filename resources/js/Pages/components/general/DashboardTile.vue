    ...
    <div class="overflow-hidden rounded-lg bg-white shadow">
            ...
            <a :href="card.href" class="group">
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            <dt class="truncate text-lg font-medium text-gray-500 group-hover:text-gray-700">{{ t(card.name) }}</dt>
                            <dd>
        ...
        <div class="bg-gray-50 px-5 py-3">
            <div class="flex text-sm justify-between">
                <a :href="card.href" class="font-medium text-cyan-700 hover:text-cyan-900">{{ t('View all') }}</a>
                <a v-if="card.alt_href" :href="card.alt_href" class="font-medium text-cyan-700 hover:text-cyan-900">{{
                    t(card.alt_link_label) }}</a>
            </div>
        </div>
    </div>
</template>
...
const styleMap = {
    ...
    PhoneOutgoingIcon: { bgColor: 'bg-amber-50', textColor: 'text-amber-700', hoverTextColor: 'group-hover:text-amber-900' },
