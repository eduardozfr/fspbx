...
const submitLead = async () => { try { await axios.post(props.routes.storeLead || '/dialer/leads', leadForm); reloadPage() } catch (error) { setMessage('error', error.response?.data?.messages?.error?.[0] || t('Unable to save the lead right now.')) } }
const handleImportFile = (event) => { importForm.file = event.target.files?.[0] || null }
const submitImport = async () => { try { const formData = new FormData(); if (importForm.file) formData.append('file', importForm.file); importForm.campaign_uuids.forEach((uuid) => formData.append('campaign_uuids[]', uuid)); if (importForm.default_state_code) formData.append('default_state_code', importForm.default_state_code); if (importForm.default_timezone) formData.append('default_timezone', importForm.default_timezone); await axios.post(props.routes.importLeads || '/dialer/imports', formData, { headers: { 'Content-Type': 'multipart/form-data' } }); reloadPage() } catch (error) { setMessage('error', error.response?.data?.messages?.error?.[0] || t('Unable to queue the import right now.')) } }
const submitDnc = async () => { try { await axios.post(props.routes.storeDnc || '/dialer/dnc', dncForm); reloadPage() } catch (error) { setMessage('error', error.response?.data?.messages?.error?.[0] || t('Unable to save the do-not-call entry right now.')) } }
const deleteDnc = async (uuid) => { try { await axios.delete(routeFor(props.routes.destroyDnc || '/dialer/dnc/__ENTRY__', '__ENTRY__', uuid)); reloadPage() } catch (error) { setMessage('error', error.response?.data?.messages?.error?.[0] || t('Unable to delete the do-not-call entry right now.')) } }
</script>
