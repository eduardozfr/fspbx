...
const submitComplianceProfile = async () => { try { const response = await axios[editingComplianceProfileUuid.value ? 'put' : 'post'](editingComplianceProfileUuid.value ? interpolateRoute(props.routes.updateComplianceProfile || '/dialer/compliance-profiles/__PROFILE__', '__PROFILE__', editingComplianceProfileUuid.value) : (props.routes.storeComplianceProfile || '/dialer/compliance-profiles'), profileForm); succeedAndReload(response.data?.messages?.success?.[0] || t(editingComplianceProfileUuid.value ? 'Compliance profile updated successfully.' : 'Compliance profile saved successfully.')) } catch (error) { setMessage('error', error.response?.data?.messages?.error?.[0] || t('Unable to save the compliance profile right now.')) } }
const deleteComplianceProfile = async (uuid) => { if (!window.confirm(t('Delete this compliance profile?'))) return; try { const response = await axios.delete(interpolateRoute(props.routes.destroyComplianceProfile || '/dialer/compliance-profiles/__PROFILE__', '__PROFILE__', uuid)); succeedAndReload(response.data?.messages?.success?.[0] || t('Compliance profile deleted successfully.')) } catch (error) { setMessage('error', error.response?.data?.messages?.error?.[0] || t('Unable to delete the compliance profile right now.')) } }
...
const submitStateRule = async () => { try { const response = await axios[selectedStateRuleUuid.value ? 'put' : 'post'](selectedStateRuleUuid.value ? interpolateRoute(props.routes.updateStateRule || '/dialer/state-rules/__STATE_RULE__', '__STATE_RULE__', selectedStateRuleUuid.value) : (props.routes.storeStateRule || '/dialer/state-rules'), stateRuleForm); succeedAndReload(response.data?.messages?.success?.[0] || t('State dialing rule saved successfully.')) } catch (error) { setMessage('error', error.response?.data?.messages?.error?.[0] || t('Unable to save the state dialing rule right now.')) } }
</script>
