...
class DialerController extends Controller
{
    ...
    public function index()
    {
        ...
        $campaigns = DialerCampaign::query()
            ...
            ->map(fn(DialerCampaign $campaign) => [
                ...
                'busy_disposition_code' => $campaign->busy_disposition_code,
                'no_answer_disposition_code' => $campaign->no_answer_disposition_code,
                'invalid_number_disposition_code' => $campaign->invalid_number_disposition_code,
    ...
    protected function validateCampaign(Request $request): array
    {
        $validated = $request->validate([
            ...
            'amd_strategy' => ['nullable', Rule::in(['avmd', 'external-webhook'])],
            'webhook_url' => [
                'nullable',
                'url',
                'max:255',
                Rule::requiredIf(fn() => $request->boolean('amd_enabled') && $request->input('amd_strategy') === 'external-webhook'),
            ],
